...
export default function CpiCalculator() {
    ...
    try {
      ...
      const semestersWithCredits = await Promise.all(
        semesterData.map(async (semester) => {
          ...
          const totalCredits = semester.subjects.reduce((sum, subject) => sum + subject.subjectCredit, 0)
  ...
  const goToPrevSemester = () => {
    if (currentSemesterIndex > 0) {
      ...
      const existingResult = semesterResults.find((result) => result.semNo === semesters[prevIndex].semesterNo)
  ...
  const saveSemesterData = () => {
    ...
    showToast(`Semester ${currentSemester.semesterNo} added successfully`, "success")
    return true
  }
  ...
  const editSemesterResult = (id) => {
    ...
    if (resultToEdit) {
      ...
      const semester = semesters.find((s) => s.semesterNo === resultToEdit.semNo)
      setCurrentSemester(semester)

      // Find the index of this semester
      const index = semesters.findIndex((s) => s.semesterNo === resultToEdit.semNo)
                    ...
                    {currentSemesterIndex < semesters.length - 1 ? (
                      <div className="next-semester">
                        <button
                          className="next-link"
                          onClick={goToNextSemester}
                          disabled={!spiValue && !isSemesterAdded(currentSemester?.semesterNo)}
                        >
